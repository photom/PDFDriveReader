...
- **Unified Physics Engine**: Flick gestures must trigger a single momentum calculation using `AnimationState.animateDecay` that handles both viewport offsets and list scroll positions synchronously. When the decayed viewport pan hits the edge, the remaining momentum is immediately passed into the `LazyList` via `dispatchRawDelta` to prevent "stuttering," "stacking," or animation cancellation when crossing page boundaries.
- **Edge-to-Edge Clamping**: When zooming in, the viewing area must be strictly clamped to the document content.
- **Inertial Panning (Smooth Fling)**: When a user releases a swipe gesture, the document must continue to move with natural momentum using a decay animation. The velocity directly maps 1:1 to screen pixels, ensuring that panning physics remain perfectly natural without any arbitrary scaling. The energy gracefully bleeds from panning the scaled bounds into scrolling the underlying list asynchronously.
