...
- **Inertial Panning (Smooth Fling)**: When a user releases a swipe gesture, the document must continue to move with natural momentum using a decay animation. The velocity directly maps 1:1 to screen pixels, ensuring that panning physics remain perfectly natural without any arbitrary scaling.
